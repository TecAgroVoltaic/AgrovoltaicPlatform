# Deploy del Historico en VisioneFlow
...
## 1. El servicio (`historico.api:app`)
...
- `POST /tool/<nombre>` — ejecuta la tool con el body JSON como params. Exige
  header `x-api-key` si `HISTORICO_API_KEY` está en el entorno.

Local:
```bash
cd agente-historico && pip install -e ".[service]"
HISTORICO_API_KEY=xxx uvicorn historico.api:app --host 0.0.0.0 --port 8000
...
- `DATABASE_URL` (o `HISTORICO_DB_URL`): Supabase PV de AgroVoltaic (Session pooler,
  usuario `postgres.<ref>`, SOLO LECTURA de facto: el servicio solo consulta).
- `HISTORICO_API_KEY`: clave fuerte; la misma va en cada `httpRequestTool` del canvas.
- `ANTHROPIC_*`: NO hace falta acá (el LLM vive en el `aiAgent` de VisioneFlow).

## 3. Docker
```bash
docker build -t historico-svc agente-historico/
docker run -p 8000:8000 -e DATABASE_URL=... -e HISTORICO_API_KEY=... historico-svc
```
En la EC2: contenedor sidecar (compose independiente, como `docker-compose.forecast.yml`)
+ location nginx **`/historico/`** → `host.docker.internal:<puerto>`. **Ojo SSRF:** el
...
- En cada `httpRequestTool`: reemplazar `PEGAR_HISTORICO_API_KEY` por la clave real, y
  ajustar la `url` base si el dominio/location difiere (`.../historico/tool/<nombre>`).
...
- ⬜ **Deploy a la EC2** (sidecar + nginx `/historico/`) e **import del flujo**: pendientes
