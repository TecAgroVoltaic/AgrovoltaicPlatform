...
## Pendiente / a validar
- ~~Overlay clear-sky~~ **HECHO 2026-06-30** (`agente-predictivo/scripts/validar_fisica.py`, ver
  `docs/predictivo/01-validacion-fisica.md`): fase/timezone OK (pico medida y clear-sky coinciden a
...
- **HECHO 2026-06-30** — forecaster de persistencia inteligente + hindcast (1.118 casos, 3,7 meses,
  ...
  hueco a atacar en Fase 2. Código: `agente-predictivo/src/predictivo/{data,physics,forecasters/persistence}.py` + `scripts/hindcast_demo.py`.
  Doc: `docs/predictivo/02-forecaster-hindcast.md`.
- **HECHO 2026-07-01** — capa del agente LLM + paquete limpio en carpeta aparte
  `agente-predictivo/` (paquete `predictivo`, SRP: config/domain/data/physics/forecasters/
  ...
  física/datos/forecaster se **migró** del viejo `src/predictivo/` preservando la lógica exacta.
  ...
  `src/predictivo/` fue eliminado; el agente vive solo en `agente-predictivo/`. Demo sin credenciales:
  `agente-predictivo/probar.py` (motor físico offline desde el caché parquet). Diagrama de arquitectura:
  `agente-predictivo/docs/arquitectura.pdf`. Contraseñas de AgroDash redactadas de [[agrodash]] (pendiente rotar).
...
Endpoint nuevo (`src/predictivo/arquitectura.py`, SRP igual que `salud.py`) que devuelve **el
...
`agente-predictivo/scripts/calibrar.py`.
