...
from predictivo import backtest as bt
...
def test_la_salida_trae_la_serie_hora_a_hora(monkeypatch):
    from predictivo.tools import backtest_tool
...
def test_el_modo_medicion_oculta_no_expone_backtest():
    ...
    from predictivo.agent.agent import MEDICION_OCULTA, MEDICION_VISIBLE, MODOS
...
def test_predecir_no_devuelve_el_valor_medido(monkeypatch):
    ...
    from predictivo.tools import predecir_tool
    monkeypatch.setattr(bt.data, "cargar_serie", lambda *a, **k: _dia_completo())
...
def test_el_diagnostico_corta_los_datos_antes_del_horizonte(monkeypatch):
    ...
    from predictivo import diagnostico
...
def test_riesgo_de_nubes_no_devuelve_el_valor_medido():
    ...
    from predictivo.tools import riesgo_tool
