...
│   └── cli.py                     ← punto de entrada: `python -m predictivo.cli "¿…?"`
...
python -m predictivo.cli "¿cuánta irradiancia va a haber en 2 horas?"
...
uvicorn predictivo.api:app --host 127.0.0.1 --port 8000

# Ingesta AgroDash -> store (idempotente e incremental)
python -m predictivo.etl                              # incremental
python -m predictivo.etl --full --variable irradiancia  # backfill de una sola variable
```
...
- **Fase 0-1 (este MVP):** lazo del agente + backtest. La física está validada y el
  ...
  (skill +0,13 a 30 min → +0,48 a 3 h). Ver `docs/predictivo/` en el repo.
