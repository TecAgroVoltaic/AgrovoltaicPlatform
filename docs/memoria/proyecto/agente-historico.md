...
# Agente Histórico (Q&A) — histórico PV de San Carlos
...
Es **distinto** del [[agente-predictivo]] (que pronostica desde AgroDash) y del Agente Histórico
...
- **Reutiliza el patrón probado del forecaster** (`agente-predictivo/`): lazo tool-use manual con
  el SDK Anthropic, DB por URL, read-only forzado, Haiku por defecto, system prompt con reglas.

## Estructura (`agente-historico/`, paquete aparte)
...
trigger + aiAgent Haiku + 6 httpRequestTool + output). Runbook: `agente-historico/DEPLOY.md`.
...
- Sidecar `analizador-analizador-1` (compose project **`analizador`**, puerto **8010**, host-net,
  ...
  `analizador.env` (DATABASE_URL + HISTORICO_API_KEY generada). El comando de compose acá es
...
**Pendiente (único):**
- **Canvas:** importar `flujo-visioneflow.json` + pegar la `HISTORICO_API_KEY` en cada httpRequestTool
