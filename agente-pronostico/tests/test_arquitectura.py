...
from pronostico import arquitectura, config, data, limites
from pronostico.agent import agent as agente_mod
from pronostico.api import ENV_API_KEY, app
from pronostico.domain import Variable

CLIENTE = TestClient(app)

# Claves del RESULTADO que el modo prediccion no puede llegar a ver por ninguna
...
def test_el_modo_prediccion_no_publica_backtest_ni_web(mapa):
    # Given: el modo donde el agente no puede conocer la respuesta
    prediccion = mapa["modos"]["prediccion"]
    ...
    assert "prediccion" not in mapa["web_search"]["modos"]


def test_backtest_queda_marcada_como_exclusiva_de_analisis(mapa):
    ...
    assert backtest["modos"] == ["analisis"]
...
def test_ninguna_herramienta_de_prediccion_pide_el_resultado(mapa):
    # Given: las herramientas del modo ciego
    del_modo = set(mapa["modos"]["prediccion"]["herramientas"])
