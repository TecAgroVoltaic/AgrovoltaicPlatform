...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figuras/fig-agente}}
...
Pronosticar la irradiancia de frente es difícil, pero por un motivo que se puede quitar de
en medio: el número lo domina la curva del sol a lo largo del día, que es pura astronomía y
se conoce de antemano. Lo que de verdad cuesta predecir son las nubes. Así que se separan
las dos cosas:
...
De esos dos factores, el segundo se calcula y el primero es el único que hay que
pronosticar.

Y no se pronostica repitiendo el $\mathrm{kt^*}$ reciente, que sería lo obvio y es lo que
se hacía al principio. Se persiste su \emph{anomalía} respecto de lo normal a esa hora del
día, trasladada a lo normal de la hora que se pronostica:
...
Son dos correcciones distintas por dos motivos distintos. La primera, porque en San Carlos
las tardes se cierran por convección: trasladar la mañana a la tarde como si fueran iguales
producía un sesgo medido de $+52$\,\% a las 16 h. La segunda, el peso $w$, porque creerle
a lo reciente vale cada vez menos a medida que el horizonte crece: la correlación del
índice consigo mismo cae de $0{,}90$ a los 5 minutos a $0{,}13$ a las 6 horas. El peso se
mide sobre el histórico, no se declara.

Reconstruir es volver a multiplicar, y ahí entra el cielo despejado del instante futuro,
que se calcula y no se adivina:

\begin{equation*}
  \mathrm{GHI_{pred}}(t+h) = \mathrm{kt^*_{pron}} \times \mathrm{GHI_{cs}}(t+h)
\end{equation*}

Ahí está la diferencia con repetir la última lectura: si la hora pronosticada cae más cerca
del mediodía, el pronóstico sube aunque las nubes sigan igual.

Para humedad de suelo no hay curva del sol que descontar. Se persiste la mediana reciente
con una banda por variabilidad, y alcanza porque el suelo cambia lento.

El detalle de cada fórmula, incluido cómo se deriva el peso y de dónde sale la banda, está
en el documento \emph{Formulario}, que las reúne todas con el archivo donde vive cada una.
