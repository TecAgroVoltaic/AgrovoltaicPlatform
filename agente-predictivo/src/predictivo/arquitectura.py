...
from predictivo import config, data, limites
from predictivo.agent import agent as _agente
...
def _catalogo_herramientas() -> list[dict]:
    ...
    marcada como exclusiva de `medicion_visible` sin que nadie lo escriba.
    """
    ...
    for nombre_modo, perfil in _agente.MODOS.items():
        for esquema in perfil["schemas"]:
            entrada = catalogo.setdefault(esquema["name"], {
                ...
                "modos": [],
                "ejecutor": "predictivo",   # corre en nuestro proceso
            })
